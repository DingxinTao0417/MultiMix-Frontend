...
| `POST /v1/video/projects/{asset_id}/reviews` | 创建独立 `film_review` 后台任务，返回 202；相同用户、工程、当前成片版本幂等复用，失败、无观测或缺失检查的报告可显式重试／补验；运行中请求不重置任务。 |
...
审片任务增加 `can_retry`、`missing_checks` 和 `report_history`。前端依据服务端投影提供“补验未完成检查”；三类观察都完成的抽样报告即使仍是 `partial` 也不重复执行。补验复用匹配真实 MP4 SHA256 与分镜时间窗口的观察检查点，只补缺失观察，并重新形成报告；检查点不匹配时重新观察。最近 20 次补验前报告和已发起的修复意图保留，失败不删除已有报告。同一成片补验不代表已修改作品，不绕过同版本禁止宣称修复的限制，且不改变导出门禁。
