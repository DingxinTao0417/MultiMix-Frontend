...
- 2026-08-22：生产工程仍未展示入口。复查确认工程、候选集及精确关联均已保存，且后端公开读取合同已补齐；历史会话仍可能带有旧工程投影。同时完成工程的分镜会引用多个素材，不能以“唯一分镜素材来源”推断候选。兼容规则调整为“精确关联优先；没有精确字段时，仅 assistant 消息精确引用且唯一的候选可恢复”。多候选会话保持隐藏入口，不使用标题、时间、素材数量或顺序猜测。
- 2026-08-22：生产复测发现消息中已有候选 `asset_id`，但会话 `products` 集合漏掉该资产，导致前端无法实际切换。后端改为按消息资产 ID 补齐同一用户的公开产品集合；前端继续只消费这一完整集合，不在浏览器端伪造候选。
- 2026-08-22：正式后端上线、前端以正确的 MultiMix 后端地址重建后，在 `111` 已完成工程实测入口出现；点击只打开既有候选页，不新建任务。候选展示区实测为 `overflow-y: auto`，`scrollHeight=2332px`、`clientHeight=732px`，可完整浏览。
