...
生产环境的编导稿已经在 `metadata.video_plan.generated_clip_recommendations` 中保存逐段关键帧建议，adapter 也把它们映射到了 `product.segments[].imageGenerationRecommendation`。但是 `app/assets/components/product-preview.tsx` 在 `product.mode === "copy"` 时直接返回 Markdown 正文，导致后面的 `SegmentCards` 永远不会渲染，用户无法采纳已经存在的建议。
...
3. 把现有 `onGenerateKeyframe` 回调传入分镜卡，沿用既有结构化采纳链路。
