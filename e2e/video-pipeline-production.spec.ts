...
test("produces six persisted visuals and recomposes only one scene", async ({ page }) => {
  ...
  const generationResponse = await postConversation(
    ...
    scenario === "hybrid"
      ...
      : scenario === "animated_public"
        ...
        : "严格基于刚上传的 MultiMix 产品资料，制作一条30秒、16:9横屏的产品介绍视频。把工作台/对话、分镜编辑/视频预览设计成两个不同的产品界面分镜，分别使用已审核产品截图，不要把整张截图直接重复铺成背景；其中至少一个界面分镜用截图作为左侧证据，右侧只补充来源中存在且不与旁白、字幕重复的信息。至少一个流程分镜使用 MG 动画补充真实步骤、差异或结构，不得重复旁白和字幕，也不得使用空泛对比项。先给出编导稿和6个分镜；信息不足按合理默认值处理，不要展示内部制作方式。",
  );
  ...
  let projectAsset = assets.filter((asset) => (
  ...
  if (expectTwoStage) {
    const persistedPipelineDecision = (projectAsset!.metadata!.video_plan as {
    internal_production?: {
      pipeline_decision?: {
  ...
  expect(
    persistedPipelineDecision?.information_increment_intent?.minimum_mg_scenes,
    "the async director path must preserve the user's explicit MG minimum",
  ).toBeGreaterThanOrEqual(1);
  expect(
    persistedPipelineDecision?.product_presentation_intent?.minimum_split_support_scenes,
  ...
  await expect.poll(async () => {
    ...
    if (rendered.length === 0) {
      throw new Error("all information-increment MG scenes reached a failed terminal state");
    }
    projectAsset = current;
    return "ready";
  }, { timeout: 15 * 60_000, intervals: [1000, 2500, 5000] }).toBe("ready");
  ...
  for (const scene of beforeScenes) {
    ...
    expect(scene.mg_brief?.params && typeof scene.mg_brief.params === "object").toBeTruthy();
  ...
  if (scenario === "hybrid") {
    ...
    if (scenario === "animated_explainer") {
      ...
      expect(new Set(productScenes.map(
        (scene) => scene.primary_visual?.provenance?.catalog_entry_id,
      ).filter(Boolean)).size).toBeGreaterThanOrEqual(2);
      const allowedVariants = new Set(["overview_frame", "focus_crop", "split_support"]);
      for (const scene of productScenes) {
        const provenance = scene.primary_visual?.provenance;
        const effectiveVariant = provenance?.effective_presentation_variant ?? "";
        expect(
        ...
        if (effectiveVariant === "focus_crop" || effectiveVariant === "split_support") {
          expect(provenance?.product_media_region_id).toBeTruthy();
        }
      }
      const splitScenes = productScenes.filter(
        (scene) => scene.primary_visual?.provenance?.effective_presentation_variant === "split_support",
      );
      expect(
        splitScenes.length,
      ...
      for (const scene of splitScenes) {
        expect(scene.primary_visual_strategy?.presentation_variant).toBe("split_support");
        expect(hasValidPresentationSupport(
  ...
  for (let index = 1; index < sortedMainElements.length; index += 1) {
    const previousEnd = Number(sortedMainElements[index - 1].startTime ?? 0)
  ...
  const mainTrackEnd = Number(sortedMainElements.at(-1)?.startTime ?? 0)
    + Number(sortedMainElements.at(-1)?.duration ?? 0);
  expect(Math.abs(mainTrackEnd - projectDuration)).toBeLessThanOrEqual(0.01);

  const supportTrack = videoProject?.tracks?.find((track) => track.id === "track-support");
  const presentationSupportElements = (mainTrack?.elements ?? []).filter(
    (element) => hasValidPresentationSupport(element.editDecision?.presentation_support),
  );
  if (expectTwoStage) {
    const decisionByScene = Object.fromEntries(
      (editDecisions?.scenes ?? []).map((decision) => [decision.scene_id, decision]),
    );
    for (const manifestScene of assetManifest?.scenes ?? []) {
      const variant = String(
        manifestScene.selected_asset?.provenance?.effective_presentation_variant ?? "",
      );
      ...
      if (variant === "split_support") {
        ...
        expect(hasValidPresentationSupport(decision?.presentation_support)).toBe(true);
      }
    }
    expect(presentationSupportElements.length).toBeGreaterThan(0);
    expect(supportTrack?.elements?.length).toBe(presentationSupportElements.length);
    for (const element of presentationSupportElements) {
      expect(element.editDecision?.layout).toBe("split");
      expect(hasValidPresentationSupport(element.editDecision?.presentation_support)).toBe(true);
  ...
  const narrationTrack = videoProject?.tracks?.find((track) => track.id === "track-audio");
  expect(new Set((narrationTrack?.elements ?? []).map((element) => element.segmentId)).size).toBe(6);
  const subtitleTrack = videoProject?.tracks?.find((track) => track.id === "track-text");
  expect(new Set((subtitleTrack?.elements ?? []).map((element) => element.segmentId)).size).toBe(6);
  const mgOverlayTrack = videoProject?.tracks?.find(
    (track) => track.id === "track-overlay" && track.overlay === true,
  );
  if (expectTwoStage) expect(mgOverlayTrack?.elements?.length).toBeGreaterThan(0);
  ...
  if (expectBgm) {
    ...
    expect(bgmTrack, "intentional no-BGM degradation must not create a music track").toBeUndefined();
  }

  const beforeRefs = Object.fromEntries(beforeScenes.map((scene) => [
    scene.id,
  ...
  const postRecomposeProject = refreshed?.metadata?.video_project as VideoProject | undefined;
  ...
  for (const element of postRecomposePresentationSupportElements) {
    ...
    afterRefs = Object.fromEntries(afterScenes.map((scene) => [
      scene.id,
      scene.primary_visual?.artifact_ref ?? projectAssets[scene.id] ?? null,
    ]));
  ...
  if (expectBgm) {
    ...
    expect(audioMix?.measurement_status ?? "not_applicable").not.toBe("measured");
  }
  await page.reload();
  await expect(page.getByLabel("分镜摘要").locator("li")).toHaveCount(6, { timeout: 120_000 });
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      productPresentation: {
        ...
        regionIds: beforeScenes.map(
          (scene) => scene.primary_visual?.provenance?.product_media_region_id,
        ).filter(Boolean),
        scenes: beforeScenes.filter(
          (scene) => scene.primary_visual?.source_type === "product_asset",
        ).map((scene) => ({
          sceneId: scene.id,
          catalogEntryId: scene.primary_visual?.provenance?.catalog_entry_id,
          variant: scene.primary_visual?.provenance?.effective_presentation_variant,
          regionId: scene.primary_visual?.provenance?.product_media_region_id,
        })),
      },
