...
assert.match(css, /\.shadcn-prototype-artifact \.shadcn-prototype-product-header \.shadcn-prototype-product-actions > button\s*\{[^}]*height:\s*var\(--sp-control-compact\);[^}]*min-height:\s*var\(--sp-control-compact\);[^}]*padding:\s*0 13px;[^}]*font-size:\s*12\.5px;/s, "header action buttons must use the compact control token and readable label size");
