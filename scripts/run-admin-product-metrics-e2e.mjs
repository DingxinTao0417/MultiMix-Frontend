...
const backendPort = Number(process.env.ADMIN_METRICS_BACKEND_PORT ?? 8297);
const frontendPort = Number(process.env.ADMIN_METRICS_FRONTEND_PORT ?? 3221);
const runId = process.env.ADMIN_METRICS_RUN_ID ?? crypto.randomUUID();
const adminEmail = "admin-metrics-e2e@multimix.local";
const adminPassword = "admin-metrics-e2e-password";

if (!Number.isInteger(backendPort) || backendPort < 1024 || backendPort > 65535) {
  throw new Error("ADMIN_METRICS_BACKEND_PORT must be an integer between 1024 and 65535");
}
if (!Number.isInteger(frontendPort) || frontendPort < 1024 || frontendPort > 65535) {
  throw new Error("ADMIN_METRICS_FRONTEND_PORT must be an integer between 1024 and 65535");
}
if (!/^[a-zA-Z0-9-]+$/.test(runId)) {
  throw new Error("ADMIN_METRICS_RUN_ID must contain only letters, numbers, and hyphens");
}

const databasePath = path.join(os.tmpdir(), `multimix-admin-product-metrics-${runId}.sqlite3`);
const artifactDir = path.join(os.tmpdir(), `multimix-admin-metrics-artifacts-${runId}`);
const resultDir = path.join(frontendRoot, "test-results", "admin-product-metrics", runId);
const nextDistDirName = `.next-admin-product-metrics-${runId}`;
...
let runError;
try {
  console.log(`Admin metrics temp database: ${databasePath}`);
  console.log(`Admin metrics temp artifacts: ${artifactDir}`);
  console.log(`Admin metrics evidence: ${resultDir}`);
  if (fs.existsSync(databasePath) || fs.existsSync(artifactDir)) {
    throw new Error(`Run id ${runId} already has temporary data; choose a fresh ADMIN_METRICS_RUN_ID`);
  }
  ...
  const backendEnv = {
    ...
    MULTIMIX_DEFAULT_ADMIN_EMAIL: adminEmail,
    MULTIMIX_DEFAULT_ADMIN_PASSWORD: adminPassword,
  ...
  await waitFor(`http://127.0.0.1:${frontendPort}/admin/product-metrics`, frontend, 180_000);

  await run(npxCommand, [
    "playwright",
    "test",
    "e2e/admin-product-metrics.spec.ts",
    "--workers",
    "1",
    "--reporter",
    "list",
    ...
    env: {
      ...frontendEnv,
      ADMIN_METRICS_BACKEND_URL: `http://127.0.0.1:${backendPort}`,
      ADMIN_METRICS_RESULT_DIR: resultDir,
      ADMIN_METRICS_RUN_ID: runId,
      ADMIN_METRICS_ADMIN_EMAIL: adminEmail,
      ADMIN_METRICS_ADMIN_PASSWORD: adminPassword,
  ...
  console.log(`Admin metrics runtime cleaned (${runError ? "after failure" : "after pass"})`);
}
